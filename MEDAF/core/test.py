...
def evaluation(model, test_loader, out_loader, **options):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, labels) in enumerate(out_loader):
            data, labels = data.cuda(), labels.cuda()
            batch_size = labels.size(0)
            ood_label = torch.zeros_like(labels) - 1  # 新类别的标签设置为 -1
    ...
    pred1, pred2 = np.argmax(pred_close, axis=1), np.argmax(pred_open, axis=1)  # 取得预测的标签
    ...
    closed_macro_f1 = f1_score(labels_close, pred1, average='macro')
    ...
    # 如果预测值大于阈值，则预测为已知类别；否则识别为未知类别(标签为 -1)
    macro_f1 = f1_score(total_label, ((total_pred_label + 1) * open_pred) - 1, average='macro')
    macro_precision = precision_score(total_label, ((total_pred_label + 1) * open_pred) - 1, average='macro')
    macro_recall = recall_score(total_label, ((total_pred_label + 1) * open_pred) - 1, average='macro')
    ...
    print('Accuracy (%): {:.3f}, '  .format(acc), 
          ...
          'Closed Set Macro F1-score: {:.5f}'  .format(closed_macro_f1)
        )
    
    return acc, auroc, aupr_in, aupr_out, macro_f1, closed_macro_f1, macro_precision, macro_recall
