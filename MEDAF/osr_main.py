...
from core import *
from misc import *
from datasets.osr_loader import CIFAR10_OSR, CIFAR100_OSR, SVHN_OSR, Tiny_ImageNet_OSR, Mal_benign_OSR
...
def getLoader(options):
    ...
    if 'cifar10' == options['dataset']:
        ...
        train_loader, test_loader, out_loader = Data.train_loader, Data.test_loader, Data.out_loader
    else:  # 每一个划分都单独作为一个数据集，统一使用 Mal_benign_OSR 处理
        options['img_size'] = 54
        Data = Mal_benign_OSR(known=options['known'], 
...
def main(options):
    ...
    known_n, unknown_n = options['dataset'].split('-')
    known_n = int(known_n)
    unknown_n = int(unknown_n)
    print('known_num:', known_n)
    print('unknown_num:', unknown_n)
    ...
    for i in range(1, exp_n + 1):
        ...
        known = list(range(0, known_n))
        unknown = list(range(known_n, known_n + unknown_n))
        ...
        stats_log.write(
            "EXP[%d|%d] => Accuracy: [%.4f], AUROC: [%.4f], AUPR_IN: [%.4f], AUPR_OUT: [%.4f], F1-score: [%.4f], Closed-set F1-score: [%.4f], Precision: [%.4f], Recall: [%.4f]\n" 
            % (i, exp_n, temp_result[0], temp_result[1], temp_result[2], temp_result[3], temp_result[4], temp_result[5], temp_result[6], temp_result[7]))
        stats_log.flush()
    stats_log.close()
...
def trainLoop(options):
    ...
    result_list = evaluation(model, test_loader, out_loader, **options)    
    print("\D-O-N-E!/ =>\nLast ACC:", result_list[0], " Last AUROC:", result_list[1],
          " Last F1-score:", result_list[4], " Last Closed Set F1-score:", result_list[5])
    return result_list
...
if __name__ == '__main__':
    ...
    set_seeding(options['seed'], deterministic=True)
